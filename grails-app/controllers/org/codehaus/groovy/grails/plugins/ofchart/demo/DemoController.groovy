...
class DemoController {
    ...
    def MANY_LINES = {
        ...
        render new Chart("Three lines example").setYAxis(new YAxis().setRange(0, 20, 5)).addElements(
                new LineChart().setWidth(4).setColour("#DFC329").setDotSize(5).addValues(data1),
                new LineChart().setWidth(1).setColour("#6363AC").setDotSize(5).addValues(data2),
                new LineChart().setWidth(1).setColour("#5E4725").setDotSize(5).addValues(data3)).toString()
    }
    ...
    def LINE_DOT = {
        ...
        def l1 = new LineChart(DOT_STYLE);
        def l2 = new LineChart(DOT_STYLE);
        def l3 = new LineChart(DOT_STYLE);
    ...
    def LINE_HOLLOW = {
        def c = new Chart(new Date().toString()).setYAxis(new YAxis().setRange(0, 15, 5));
        def l1 = new LineChart(HOLLOW_STYLE);
        def l2 = new LineChart(HOLLOW_STYLE);
        def l3 = new LineChart(HOLLOW_STYLE);
    ...
    def COMPLEX_LABELS = {
        ...
        Chart c = new Chart("X Axis Labels Complex Example").addElements(new LineChart(DOT_STYLE).addValues(9, 8, 7, 6, 5, 4, 3, 2, 1)).setXAxis(new XAxis().setTickHeight(5).addLabels("one", "two", "three", "four", "five").addLabels(
    ...
    def AREA_HOLLOW = {
        ...
        def areaChart = new AreaChart()
        AbstractDot.Style style = new AbstractDot.Style(AbstractDot.Style.Type.DOT)
        style.with {
            setDotSize 1
            setHaloSize 2
        }
        ...
        Chart c = new Chart("Area Chart").addElements(areaChart.addValues(data).setWidth(1));
    ...
    def BAR_CHART_FILLED = {
        ...
        render new Chart(new Date().toString()).addElements(new FilledBarChart().setOutlineColour("#577261").setColour("#E2D66A").addValues(9, 8, 7, 6, 5, 4, 3, 2, 1)).setBackgroundColour("#FFFFFF").toString();

    }

    def BAR_CHART_SKETCH = {

        render new Chart(new Date().toString(), "{color: #567300; font-size: 14px}").addElements(new SketchBarChart("#81AC00", "#567300", 5).addValues(9, 8, 7, 6, 5, 4, 3, 2, 1)).toString();
    ...
    def ADV_DOT_LINES = {
        ...
        def lc = new LineChart(DOT_STYLE);
        lc.setWidth(2).setColour("#DFC329").setDotSize(3);
        for (int i = 0; i < 8; i += 1) {
            Number val = Math.sin(i) + 1.5;
